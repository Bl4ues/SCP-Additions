...
public class ContextMenu {
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < options.size(); i++) {
            ...
            g.drawString(
                    ...
                    x + (MENU_WIDTH - mc.font.width(ScpFonts.roboto(label))) / 2,
                    optionY + 7,
    ...
    private List<String> wrapHintText(String text, int maxWidth) {
        ...
        for (String word : words) {
            ...
            if (mc.font.width(ScpFonts.roboto(candidate)) <= maxWidth || current.isEmpty()) {
                current.setLength(0);
    ...
    private void drawScaledString(GuiGraphics g, String text, int x, int y, int color, float scale) {
        ...
        g.drawString(mc.font, ScpFonts.roboto(text), 0, 0, color, false);
        g.pose().popPose();
    }
