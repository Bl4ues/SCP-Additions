...
public class Scp1176honeyPlayerFinishesUsingItemProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if ((entity.getCapability(ScpClassifiedDirectiveModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new ScpClassifiedDirectiveModVariables.PlayerVariables())).ABpos) {
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide()) {
				_entity.addEffect(new MobEffectInstance(MobEffects.SATURATION, 12000, 10, false, false));
				_entity.addEffect(new MobEffectInstance(ScpClassifiedDirectiveModMobEffects.SCP_1176_HONEYED.get(), 12000, 0, false, false, false));
			}
			...
			if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide()) {
				...
				_entity.addEffect(new MobEffectInstance(ScpClassifiedDirectiveModMobEffects.SCP_1176_HONEYED.get(), 1360, 0, false, false, false));
			}
			ScpClassifiedDirectiveMod.queueServerWork(1360, () -> {
				if (entity instanceof LivingEntity _entity)
