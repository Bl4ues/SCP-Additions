...
"""
    ...
    for source in JAVA.rglob("*.java"):
        if source == path:
            continue
        text = read(source)
        updated = text.replace("ScpInventoryConfig.reload();", "ScpInventoryConfig.reloadFromDisk();")
        if updated != text:
            write(source, updated)
            changed += text.count("ScpInventoryConfig.reload();")
            print(f"updated local inventory reload: {source.relative_to(ROOT)}")
    if changed == 0:
        raise RuntimeError("No server/local ScpInventoryConfig.reload() call was updated")
...
"""
    ...
    for source in JAVA.rglob("*.java"):
        if source == path:
            continue
        text = read(source)
        updated = text.replace("ContextInteractionRegistry.reload();", "ContextInteractionRegistry.reloadFromDisk();")
        if updated != text:
            write(source, updated)
            changed += text.count("ContextInteractionRegistry.reload();")
            print(f"updated local context reload: {source.relative_to(ROOT)}")
    if changed == 0:
        raise RuntimeError("No server/local ContextInteractionRegistry.reload() call was updated")
