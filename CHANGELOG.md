...
- SCP Inventory can now be closed with its configured open key, matching vanilla inventory-toggle behavior;
- Placeable items now support the same quick hotbar action as Usable items through double-click and the contextual action, and selecting another item from an already-occupied hotbar category replaces the previous item instead of leaving the old session active;
- Removed the portable Crafting tab, its normal Minecraft recipe support, related networking and UI resources, and bundled normal crafting recipes; SCP: Classified Directive currently does not use vanilla crafting as an item-acquisition path;
