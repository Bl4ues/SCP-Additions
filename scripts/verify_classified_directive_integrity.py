#!/usr/bin/env python3
"""Static guardrails for the SCP: Classified Directive rebrand.

This deliberately distinguishes dangerous stale namespace/package references
from legacy persistence identifiers which must remain readable for old worlds.
It performs no source mutation.
"""
...
def whats_new_highlights() -> list[str]:
    ...
    for raw in match.group("body").splitlines():
        ...
        if not (raw.startswith('"') and raw.endswith('"')):
            continue
        # The current list uses ordinary Java string literals whose escapes are
        # compatible with JSON string decoding.
...
def main() -> None:
    ...
    print("SCP: Classified Directive integrity verification passed.")
