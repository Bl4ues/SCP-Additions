package com.bl4ues.scpclassifieddirective.inventory.item;
...
public final class ScpItemClassifier {
    ...
    public static ScpItemType getType(ItemStack stack) {
        ...
        if (isCanonicalScpClassifiedDirectiveCoin(stack)) {
            return ScpItemType.MISCELLANEOUS;
        }
        ...
        if (isThrownPotion(stack)) return ScpItemType.USABLE;
        if (isDefaultConsumable(stack)) return ScpItemType.CONSUMABLE;
        if (stack.getItem() instanceof ArmorItem armorItem) {
            return fromVanillaEquipmentSlot(armorItem.getEquipmentSlot());
        }
        if (stack.getItem() instanceof BlockItem) return ScpItemType.PLACEABLE;
    ...
    public static boolean isCoin(ItemStack stack) {
        return stack != null && !stack.isEmpty()
                ...
                && (isCanonicalScpClassifiedDirectiveCoin(stack)
    ...
    public static boolean isMirroredMainItem(ItemStack stack) {
        if (stack == null || stack.isEmpty()
                || ScpPickupRouter.isCoinMirror(stack)
                || ScpPickupRouter.isHarmfulMirror(stack)) return false;
        if (isCanonicalScpClassifiedDirectiveCoin(stack)) return true;
    ...
    public static boolean isConfiguredMirroredMainItem(ItemStack stack) {
        ...
        for (String rawRule : ScpInventoryConfig.itemRules()) {
            ...
            if (rule.isPresent() && rule.get().itemId().equals(stackId)
                    && isMirroredMainType(rule.get().type())) return true;
        }
        return false;
    }
    ...
    public static Optional<CodexDocumentDefinition> getCodexDocument(ItemStack stack) {
        ...
        // A configured unique definition may still use the dedicated item as
        // its physical carrier. Structured item data takes precedence only
        // after the player has edited that item directly.
        if (DocumentData.hasStructuredData(stack)) {
            return Optional.of(CodexDocumentDefinition.dedicated(stack));
        }

        List<CodexDocumentDefinition> genericMatches = new ArrayList<>();
        for (String rawRule : ScpInventoryConfig.codexDocuments()) {
            Optional<CodexDocumentDefinition> parsed = CodexDocumentDefinition.parse(rawRule);
            if (parsed.isEmpty() || !parsed.get().matches(stack)) continue;
            if (parsed.get().isUnique()) return parsed;
            genericMatches.add(parsed.get());
        }
        if (!genericMatches.isEmpty()) return Optional.of(genericMatches.get(0));

        if (DocumentData.isDedicatedItem(stack)) {
            return Optional.of(CodexDocumentDefinition.dedicated(stack));
        }
        return Optional.empty();
    }
    ...
    private static boolean isThrownPotion(ItemStack stack) {
        ...
        if (item instanceof SplashPotionItem || item instanceof LingeringPotionItem) return true;
    ...
    private static boolean isDefaultConsumable(ItemStack stack) {
        if (isThrownPotion(stack)) return false;
        ...
        return path.equals("potion") || path.endsWith("_potion") || path.contains("potion");
    }
    ...
    private static boolean isDefaultUsable(ItemStack stack) {
        if (isThrownPotion(stack)) return true;
        Item item = stack.getItem();
        if (item instanceof BlockItem) return false;
        UseAnim animation = stack.getUseAnimation();
        if (animation == UseAnim.BLOCK || animation == UseAnim.SPYGLASS
                || animation == UseAnim.TOOT_HORN || animation == UseAnim.BRUSH) return true;
        if (item instanceof FishingRodItem || item instanceof FlintAndSteelItem
                || item instanceof ShearsItem || item instanceof BucketItem
                || item instanceof EnderpearlItem || item instanceof SnowballItem
                || item instanceof EggItem || item instanceof ExperienceBottleItem
                || item instanceof FireworkRocketItem) return true;
    ...
    private static boolean overridesAirUse(Item item) {
        return AIR_USE_OVERRIDES.computeIfAbsent(item.getClass(), itemClass -> {
            try {
                Method use = itemClass.getMethod("use", Level.class, Player.class,
    ...
    private static Optional<ScpItemType> getConfiguredType(ItemStack stack) {
        ResourceLocation stackId = BuiltInRegistries.ITEM.getKey(stack.getItem());
        if (stackId == null) return Optional.empty();
        for (String rawRule : ScpInventoryConfig.itemRules()) {
            Optional<ConfiguredItemRule> rule = parseItemRule(rawRule);
            if (rule.isPresent() && rule.get().itemId().equals(stackId)) {
