...
Path("src/main/java/com/bl4ues/scpinventory/events/InventoryModuleStateEvents.java").write_text('''package com.bl4ues.scpinventory.events;
...
old = "- Fixed `inventory.enabled: false` still allowing background tick handlers and gameplay packets to move items into the SCP Inventory; the server now synchronizes this module state to clients, blocks stale actions, and closes the custom screen when disabled;\n"
...
path.write_text(replace_once(text, old, new, "changelog migration note"), encoding="utf-8")
