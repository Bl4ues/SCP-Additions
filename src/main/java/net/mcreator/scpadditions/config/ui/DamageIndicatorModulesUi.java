...
/** Adds the personal damage-indicator particle preference to General & Modules. */
@Mod.EventBusSubscriber(modid = ScpAdditionsMod.MODID, value = Dist.CLIENT)
public final class DamageIndicatorModulesUi {
    ...
    private static final String LABEL = "Hide Damage Indicator Particles";

    private DamageIndicatorModulesUi() {
    }
    ...
    public static void onInitPre(ScreenEvent.Init.Pre event) {
        ...
        try {
            ...
            int insertion = indexAfterLabel(rows,
                    "Hide Active Effect Indicators");
            expanded.add(insertion, constructor.newInstance(
                    "hud", "hide_damage_indicator_particles", LABEL,
                    "Hides vanilla heart-like damage indicator particles emitted when living entities are hit.",
                    true));
            rowsField.set(screen, List.copyOf(expanded));
        } catch (ReflectiveOperationException exception) {
            ScpAdditionsMod.LOGGER.warn(
                    "Could not add the damage indicator particle preference row",
                    exception);
        }
    }
