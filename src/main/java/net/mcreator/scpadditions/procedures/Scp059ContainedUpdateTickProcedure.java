...
public class Scp059ContainedUpdateTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		ScpAdditionsMod.queueServerWork(18000, () -> {
			ScpAdditionsModVariables.MapVariables.get(world).RandomX = x + Mth.nextDouble(RandomSource.create(), -3, 3);
			ScpAdditionsModVariables.MapVariables.get(world).syncData(world);
			ScpAdditionsModVariables.MapVariables.get(world).RandomY = y + Mth.nextDouble(RandomSource.create(), -3, 3);
			ScpAdditionsModVariables.MapVariables.get(world).syncData(world);
			ScpAdditionsModVariables.MapVariables.get(world).RandomZ = z + Mth.nextDouble(RandomSource.create(), -3, 3);
			ScpAdditionsModVariables.MapVariables.get(world).syncData(world);
		...
		});
		{
			...
			for (Entity entityiterator : _entfound) {
				if (!world.getEntitiesOfClass(LivingEntity.class, AABB.ofSize(new Vec3(x, y, z), 10, 10, 10), e -> true).isEmpty()) {
					...
					if (!world.getEntitiesOfClass(LivingEntity.class, AABB.ofSize(new Vec3(x, y, z), 20, 20, 20), e -> true).isEmpty()) {
						...
						if (!world.getEntitiesOfClass(LivingEntity.class, AABB.ofSize(new Vec3(x, y, z), 40, 40, 40), e -> true).isEmpty()) {
							if (ScpAdditionsModItems.GEIGER_1.get() == (entityiterator instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()
