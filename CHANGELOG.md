...
- Added a default-enabled client-side **Facility Chat Interface** that keeps vanilla chat behavior and settings while moving the message flow to the upper-left, presenting history top-to-bottom in an SCP Unity / Configuration Center-inspired framed communications panel, relocating the input and command suggestions with it, and restoring the exact vanilla chat presentation when disabled;
