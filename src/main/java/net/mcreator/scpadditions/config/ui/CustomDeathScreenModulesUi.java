...
/** Adds the client-only Custom Death Screen row to General & Modules. */
@Mod.EventBusSubscriber(modid = ScpAdditionsMod.MODID, value = Dist.CLIENT)
public final class CustomDeathScreenModulesUi {
    ...
    private static final String LABEL = "Custom Death Screen";

    private CustomDeathScreenModulesUi() {
    }
    ...
    public static void onInitPre(ScreenEvent.Init.Pre event) {
        ...
        try {
            ...
            expanded.add(insertion, constructor.newInstance(
                    ...
                    "Replaces Minecraft's death screen with the SCP Additions termination report.",
                    true));
            rowsField.set(screen, List.copyOf(expanded));
        } catch (ReflectiveOperationException exception) {
            ScpAdditionsMod.LOGGER.warn(
                    "Could not add the Custom Death Screen module row", exception);
        }
    }
