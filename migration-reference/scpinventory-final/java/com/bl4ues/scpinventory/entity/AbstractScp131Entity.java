...
public abstract class AbstractScp131Entity extends PathfinderMob implements GeoEntity {
    ...
    protected InteractionResult mobInteract(Player player, InteractionHand hand) {
        if (!level().isClientSide) {
            ...
            if (player instanceof ServerPlayer serverPlayer) {
                ModNetwork.showScp131Notice(serverPlayer, true);
            }
            playVoice(1.0F);
    ...
    public static boolean stopFollowersFor(Player player) {
        ...
        for (AbstractScp131Entity scp131 : player.level().getEntitiesOfClass(AbstractScp131Entity.class, area, entity -> entity.isAlive() && entity.isFollowingPlayer(player))) {
    ...
    private boolean followNearbyScp131Leader() {
        ...
        if (distanceSqr > COMPANION_FOLLOW_DISTANCE_SQR || getNavigation().isDone() || tickCount >= nextOwnerRoamTick) {
            ...
            getNavigation().moveTo(spot.x, spot.y, spot.z, distanceSqr > COMPANION_FOLLOW_DISTANCE_SQR ? COMPANION_CATCH_UP_SPEED : COMPANION_CLOSE_SPEED);
            scheduleNextOwnerRoam(OWNER_ROAM_INTERVAL_TICKS + random.nextInt(OWNER_ROAM_RANDOM_TICKS + 1));
        }
    ...
    private void playVoice(float volume) {
        level().playSound(null, getX(), getY() + 0.35D, getZ(), ModSounds.EYE_POD_VOICE.get(), SoundSource.NEUTRAL, volume,
                0.86F + (random.nextFloat() * 0.30F));
    }
