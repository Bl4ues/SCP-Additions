...
public class EquipmentPanel {
    ...
    private void renderSlot(GuiGraphics g, ScpEquipmentSlot slot, int rowY) {
        ...
        g.drawString(mc.font, ScpFonts.roboto(slot.getDisplayName()), textX, rowY + 7, TEXT_WHITE, false);
        g.drawString(
                mc.font,
                ScpFonts.roboto(itemName),
                textX,
                rowY + 20,
    ...
    private void drawSectionTitle(GuiGraphics g, int x, int y, String suffix) {
        ...
        g.drawString(mc.font, ScpFonts.roboto(suffix), x + mc.font.width(ScpFonts.roboto(prefix)), y, TEXT_WHITE, false);
    }
}
