...
import com.bl4ues.scpclassifieddirective.facility.AreaUnderConstructionSignBlock;
import com.bl4ues.scpclassifieddirective.facility.AreaUnderConstructionSignBlockEntity;
import org.joml.Matrix3f;
import org.joml.Matrix4f;

/** Draws the construction-area artwork directly behind the support glass. */
public final class AreaUnderConstructionSignBlockEntityRenderer
        implements BlockEntityRenderer<AreaUnderConstructionSignBlockEntity> {
    ...
    public AreaUnderConstructionSignBlockEntityRenderer(
            BlockEntityRendererProvider.Context context) {
    }

    @Override
    public void render(AreaUnderConstructionSignBlockEntity sign,
            float partialTick, PoseStack poseStack, MultiBufferSource buffer,
            int packedLight, int packedOverlay) {
        BlockState state = sign.getBlockState();
        if (!(state.getBlock() instanceof AreaUnderConstructionSignBlock)) {
            return;
        }
        ...
        vertex(consumer, matrix, normal, PANEL_MAX_X, PANEL_MAX_Y,
                0.0F, 0.0F, packedLight);
        vertex(consumer, matrix, normal, PANEL_MAX_X, PANEL_MIN_Y,
                0.0F, 1.0F, packedLight);
        vertex(consumer, matrix, normal, PANEL_MIN_X, PANEL_MIN_Y,
                1.0F, 1.0F, packedLight);
        vertex(consumer, matrix, normal, PANEL_MIN_X, PANEL_MAX_Y,
