...
public class Scp173Entity extends BlinkWatcherEntity {
    ...
    private static final ResourceKey<DamageType> NECK_SNAP_DAMAGE_TYPE = ResourceKey.create(Registries.DAMAGE_TYPE, new ResourceLocation(ScpInventoryMod.MODID, "scp_173_neck_snap"));
    ...
    private FrozenPose clientObservedVisualLock = null;
    private int lastSeenOrCloseTick = 0;
    private int nextAttackTick = 0;
    private int nextPathRecalculationTick = 0;
    private double frozenFallSpeed = 0.0D;
    ...
    public static void reactToBlinkState(ServerPlayer player, boolean closed) {
        ...
        for (Scp173Entity scp173 : player.serverLevel().getEntitiesOfClass(Scp173Entity.class, area,
            ...
            if (!scp173.isActivated() && !scp173.isObservedBy(player)) {
                continue;
            }
    ...
    public void tick() {
        ...
        if (target != null) {
            if (target instanceof Player && distanceToSqr(target) <= DESPAWN_DISTANCE_SQR) {
                lastSeenOrCloseTick = tickCount;
            }
            if (!trySnapAttack(target)) {
                reactImmediatelyToTarget(target);
            }
        } else {
            stopAndLock(preTickPose);
        }
    ...
    public boolean isScraping() {
        return entityData.get(SCRAPING);
    }
    ...
    public boolean isRoutineSpawn() {
        return entityData.get(ROUTINE_SPAWN);
    }

    public boolean isObservedBy(Player player) {
        return shouldFreezeFor(player);
    }

    @Override
    public boolean doHurtTarget(Entity entity) {
        if (level().isClientSide || !(entity instanceof LivingEntity target) || !canSnapTarget(target)) {
            return false;
        }
    ...
    public void die(DamageSource source) {
        if (!level().isClientSide) {
            level().playSound(null, getX(), getY(), getZ(), ModSounds.STATUE_DEATH.get(), SoundSource.HOSTILE, 1.0F, 1.0F);
        }
        super.die(source);
    }
    ...
    private void setActivated(boolean value) {
        entityData.set(ACTIVATED, value);
    }
    ...
    private boolean shouldFreezeFor(LivingEntity observer) {
        ...
        if (observer instanceof Player player && BlinkServerState.isBlinkClosed(player)) {
            return false;
        }
    ...
    private boolean isValidObserver(LivingEntity entity) {
        if (entity == null || entity == this || !entity.isAlive() || entity.isRemoved()) {
            return false;
        }
    ...
    private LivingEntity resolveTarget() {
        ...
        if (isValidTargetEntity(currentTarget)) {
    ...
    private boolean hasClosePlayer() {
        for (Player player : level().players()) {
            if (!player.isCreative() && !player.isSpectator() && distanceToSqr(player) <= DESPAWN_DISTANCE_SQR) {
                return true;
            }
        }
        return false;
    ...
    private void handleRoutineDespawn() {
        if (!isRoutineSpawn() || level().isClientSide) {
            return;
        }
        if (findObservingPlayer() != null || hasClosePlayer()) {
            return;
        }
    ...
    private boolean isClientObservedByLocalPlayer() {
        ...
        if (mc.player == null || mc.player.isCreative() || mc.player.isSpectator()
                || com.bl4ues.scpinventory.client.BlinkClient.isBlinkClosedLocally()) {
            return false;
        }
    ...
    private boolean isObservedGeometry(LivingEntity observer) {
        ...
        double minX = box.minX + VISIBILITY_EPSILON;
        double midX = (box.minX + box.maxX) * 0.5D;
        double maxX = box.maxX - VISIBILITY_EPSILON;
        double minY = box.minY + VISIBILITY_EPSILON;
        double midY = (box.minY + box.maxY) * 0.5D;
        double maxY = box.maxY - VISIBILITY_EPSILON;
        double minZ = box.minZ + VISIBILITY_EPSILON;
        double midZ = (box.minZ + box.maxZ) * 0.5D;
        double maxZ = box.maxZ - VISIBILITY_EPSILON;
    ...
    private boolean isVisibleSample(Vec3 eye, Vec3 look, Vec3 point) {
        ...
        if (dot < OBSERVED_DOT_THRESHOLD) {
            return false;
        }
        return hasVisualLineOfSightThroughTransparentBlocks(eye, point);
    }
    ...
    private void freezeClientAtObservedPosition() {
        if (clientObservedVisualLock == null) {
    ...
    private void reactImmediatelyToTarget(LivingEntity target) {
        ...
        if (findObservingEntity() != null) {
            ...
            chaseImmediately(target, preActionPose);
        }
    }
    ...
    private void applyFrozenVerticalPhysics() {
        if (isInWater()) {
            frozenFallSpeed = Math.max(frozenFallSpeed - FROZEN_WATER_GRAVITY, FROZEN_MAX_WATER_SINK_SPEED);
        } else if (!onGround()) {
            frozenFallSpeed = Math.max(frozenFallSpeed - FROZEN_AIR_GRAVITY, FROZEN_MAX_AIR_FALL_SPEED);
        } else {
            frozenFallSpeed = 0.0D;
            return;
        }

        move(MoverType.SELF, new Vec3(0.0D, frozenFallSpeed, 0.0D));
        if (onGround() && frozenFallSpeed < 0.0D) {
            frozenFallSpeed = 0.0D;
        }
    }
    ...
    private void applyHeavyWaterSinking() {
        if (isInWater()) {
            move(MoverType.SELF, new Vec3(0.0D, -0.08D, 0.0D));
        }
    }
    ...
    private Vec3 chooseChaseStep(LivingEntity target, Vec3 directHorizontal, double distance, double maxStep) {
        ...
        if (stepDistance <= 0.001D || directHorizontal.lengthSqr() <= 0.000001D) {
            return Vec3.ZERO;
        }
        ...
        if (pathStep.lengthSqr() > 0.000001D && canMoveBy(pathStep)) {
            return pathStep;
        }
    ...
    private Vec3 pathStepToward(LivingEntity target, double stepDistance) {
        if (tickCount >= nextPathRecalculationTick || getNavigation().getPath() == null || getNavigation().isDone()) {
            ...
            if (path != null) {
                getNavigation().moveTo(path, 1.25D);
            }
        ...
        Vec3 toNode = next.subtract(position());
        Vec3 horizontal = new Vec3(toNode.x, 0.0D, toNode.z);
        if (horizontal.lengthSqr() <= PATH_NODE_REACHED_DISTANCE_SQR) {
            ...
            toNode = next.subtract(position());
            horizontal = new Vec3(toNode.x, 0.0D, toNode.z);
        }
        ...
        return horizontal.scale(1.0D / length).scale(Math.min(stepDistance, length));
    }
    ...
    private Vec3 bestFallbackStep(LivingEntity target, Vec3 directHorizontal, double stepDistance) {
        ...
        double[] angles = new double[]{35.0D, -35.0D, 70.0D, -70.0D, 110.0D, -110.0D, 160.0D, -160.0D};
        Vec3 best = Vec3.ZERO;
        double bestScore = Double.MAX_VALUE;
        for (double angle : angles) {
            Vec3 candidate = rotateHorizontal(base, angle).scale(stepDistance * 0.75D);
            if (!canMoveBy(candidate)) {
                continue;
            }
    ...
    private boolean trySnapAttack(LivingEntity target) {
        if (canSnapTarget(target)) {
            return snapTargetNeck(target);
        }
        return false;
    }
    ...
    private boolean isInSnapRange(LivingEntity target) {
        return target != null
                && hasVerticalAttackOverlap(target)
    ...
    private boolean hasVerticalAttackOverlap(LivingEntity target) {
        return getBoundingBox().maxY >= target.getBoundingBox().minY
                && getBoundingBox().minY <= target.getBoundingBox().maxY;
    }

    private boolean canSnapTarget(LivingEntity target) {
        return isActivated()
                && isValidTargetEntity(target)
                && tickCount >= nextAttackTick
                && isInSnapRange(target)
                && findObservingEntity() == null;
    }
    ...
    private void faceTarget(LivingEntity target) {
        ...
        setManualYaw(yawTo(toTarget));
    }

    private float yawTo(Vec3 toTarget) {
        return (float) (Mth.atan2(toTarget.z, toTarget.x) * Mth.RAD_TO_DEG) - 90.0F;
    }

    private void setManualYaw(float yaw) {
        yaw = Mth.wrapDegrees(yaw);
        entityData.set(MANUAL_YAW, yaw);
        applyYaw(yaw);
    }
    ...
    private void applyClientManualRotation() {
        if (isScraping()) {
            ...
            if (mc.player != null) {
                ...
                if ((toPlayer.x * toPlayer.x) + (toPlayer.z * toPlayer.z) > 0.000001D) {
                    applyYaw(yawTo(toPlayer));
                    return;
                }
            }
        }
    ...
    private void playNeckSnapSound() {
        level().playSound(null, getX(), getY(), getZ(), ModSounds.NECK_SNAP.get(), SoundSource.HOSTILE, 1.0F, 0.96F + (getRandom().nextFloat() * 0.08F));
    }

    private FrozenPose capturePose() {
        return new FrozenPose(getX(), getY(), getZ(), getXRot(), getYRot());
    }
