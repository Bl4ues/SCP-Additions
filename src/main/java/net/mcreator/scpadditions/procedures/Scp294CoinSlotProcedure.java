		...
		if (ScpAdditionsModVariables.WorldVariables.get(world).coinslot == 0
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(ResourceLocation.parse("scp_additions:scp294coinslot")), SoundSource.NEUTRAL, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(ResourceLocation.parse("scp_additions:scp294coinslot")), SoundSource.NEUTRAL, 1, 1, false);
				}
			}
		}
	}
