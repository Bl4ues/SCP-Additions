			...
			for (Entity entityiterator : _entfound) {
				if (!(entityiterator.level().clip(new ClipContext(entityiterator.getEyePosition(1f), entityiterator.getEyePosition(1f).add(entityiterator.getViewVector(1f).scale(5)), ClipContext.Block.VISUAL, ClipContext.Fluid.NONE, entityiterator))
						.getType() == HitResult.Type.BLOCK)) {
					{
						...
						BlockState _bs = ScpClassifiedDirectiveModBlocks.SCP_079OFF.get().defaultBlockState();
					...
					if (world instanceof Level _level) {
						if (!_level.isClientSide()) {
							_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("scp_classified_directive:scp079_1")), SoundSource.HOSTILE, 1, 1);
						} else {
							_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("scp_classified_directive:scp079_1")), SoundSource.HOSTILE, 1, 1, false);
						}
					}
				}
			}
