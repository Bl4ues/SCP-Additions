...
import com.bl4ues.scpclassifieddirective.network.BlinkInputStatePacket;

public final class BlinkClient {
    private static final ResourceLocation EYE_ICON = new ResourceLocation("scp_classified_directive", "textures/gui/eye.png");
    private static final ResourceLocation VIGNETTE = new ResourceLocation("scp_classified_directive", "textures/gui/vignette.png");
    ...
    public static void setActive(boolean value) {
        value &= ScpClassifiedDirectiveModulesConfig.get().blink.enabled;
    ...
    public static boolean isBlinkClosedLocally() {
        return ScpClassifiedDirectiveModulesConfig.get().blink.enabled
                && active
    ...
    public static void clientTick() {
        ...
        if (!ScpClassifiedDirectiveModulesConfig.get().blink.enabled) {
            setActive(false);
            darkTicks = 0;
            postBlinkCoverTicks = 0;
            visualAlpha = 0.0F;
    ...
    private static boolean hasEyeSore(Minecraft mc) {
        return mc != null && mc.player != null && mc.player.hasEffect(ScpClassifiedDirectiveModMobEffects.EYE_SORE.get());
    }

    private static boolean hasLubricatedEye(Minecraft mc) {
        return mc != null && mc.player != null
                && mc.player.hasEffect(ScpClassifiedDirectiveModMobEffects.LUBRICATED_EYE.get());
    }
    ...
    private static void syncBlinkClosed(boolean closed, boolean force) {
        ...
        ScpClassifiedDirectiveMod.PACKET_HANDLER.sendToServer(new BlinkInputStatePacket(closed, manual));
    }
